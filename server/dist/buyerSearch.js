...
exports.searchBuyers = searchBuyers;
...
function scoreBuyer(tokens, b) {
    ...
    for (const t of tokens) {
        let tokenScore = 0;
        if (includesCI(name, t))
            tokenScore = Math.max(tokenScore, 4.0);
        if (includesCI(tags, t))
            tokenScore = Math.max(tokenScore, 2.8);
        if (includesCI(sectors, t))
            tokenScore = Math.max(tokenScore, 2.3);
        if (includesCI(geos, t))
            tokenScore = Math.max(tokenScore, 1.7);
        if (includesCI(type, t))
            tokenScore = Math.max(tokenScore, 1.2);
        if (tokenScore > 0)
            hits.push(t);
        score += tokenScore;
    }
...
function searchBuyers(params) {
    ...
    const filtered = buyers_1.BUYERS.filter((b) => {
        if (sector && !b.sectorFocus.includes(sector))
            return false;
        if (type && b.type !== type)
            return false;
        if (geo && !b.geographies.some((g) => includesCI(geo, g) || includesCI(g, geo)))
            return false;
        if (tag && !b.strategyTags.some((t) => includesCI(t, tag)))
            return false;
        // mandate-ish numeric filters (overlap)
        if (minDeal != null && b.maxDealSize < minDeal)
            return false;
        if (maxDeal != null && b.minDealSize > maxDeal)
            return false;
        if (minEbitda != null && b.maxEbitda < minEbitda)
            return false;
        if (maxEbitda != null && b.minEbitda > maxEbitda)
            return false;
        return true;
    });
    const scored = filtered
        ...
        const parts = [];
        if (hits.length)
            parts.push(`Matched: ${hits.slice(0, 6).join(", ")}`);
        if (!hits.length && q)
            parts.push("No keyword hits; matched via filters");
        const m = mandateReason(b, params);
        if (m.length)
            parts.push(...m);
        if (b.strategyTags?.length)
            parts.push(`Tags: ${b.strategyTags.slice(0, 4).join(", ")}`);
        return {
            b,
            finalScore,
    ...
    const final = isEmptySearch
        ? buyers_1.BUYERS.slice().sort((a, b) => (b.pastDeals || 0) - (a.pastDeals || 0)).slice(0, limit).map((b) => ({
            id: b.id,
            name: b.name,
            type: b.type,
